...
It loads a Tesla earnings PDF, splits the text into chunks, stores embeddings in a
local Chroma vector database, retrieves the most relevant chunks for a question,
and sends that context to an AWS Bedrock Claude model.

## What It Uses

- Python
- ChromaDB for local vector storage
- Chroma default embedding function for local embeddings
...
You need access to this model enabled in the AWS Bedrock console.
...
4. Embeds each chunk using Chroma's default embedding function
5. Stores the chunks in `./db/Tesla_earnings_db`
